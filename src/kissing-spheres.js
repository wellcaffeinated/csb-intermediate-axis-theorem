...
export function createKissingSpheres(resolution = 128) {
  ...
  const update = (Escale, L, w, m1, m2) => {
    ...
    const I3 = M
    const Lmag = L.length()
    const router = 1 / I2 - 1 / I3
    const rupper = 1 / I2 - 1 / I1
    const rlower = 1 / I1 - 1 / I3
    outer.scale.set(router, router, router)
    upper.scale.set(rupper, rupper, rupper)
    lower.scale.set(rlower, rlower, rlower)
    outer.position.copy(L).multiplyScalar((1 / I2 + 1 / I3) / Lmag)
    upper.position.copy(L).multiplyScalar((1 / I2 + 1 / I1) / Lmag)
    lower.position.copy(L).multiplyScalar((1 / I1 + 1 / I3) / Lmag)
  }
